...
export default function AuctionCard({ auction }: AuctionCardProps) {
  ...
  const isEndingSoon = !isEnded && new Date(auction.endTime).getTime() - Date.now() < 3600000;
  
  const winnerName = typeof auction.highestBidderId === 'object' 
  ...
  return (
    <Link 
      ...
      style={{ 
        ...
        border: isEnded ? '1px solid rgba(255,255,255,0.05)' : '1px solid rgba(255,255,255,0.1)', 
        borderRadius: '24px', 
        overflow: 'hidden', 
    ...
    >
      ...
      <div style={{ position: 'relative', paddingTop: '75%', backgroundColor: '#111', borderBottom: '1px solid rgba(255,255,255,0.05)', overflow: 'hidden' }}>
        {auction.images && auction.images[0] ? (
          <img
            ...
            style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', objectFit: 'cover', filter: isEnded ? 'grayscale(0.5)' : 'none' }}
          />
        ) : (
        ...
        <div style={{ position: 'absolute', top: '16px', left: '16px', display: 'flex', gap: '8px', zIndex: 10 }}>
          ...
          {isEnded ? (
            <span style={{ backgroundColor: 'rgba(239, 68, 68, 0.9)', color: '#fff', fontSize: '10px', padding: '4px 10px', borderRadius: '6px', textTransform: 'uppercase', fontWeight: 800, letterSpacing: '0.05em', border: '1px solid rgba(239, 68, 68, 0.5)' }}>
              Ended
            </span>
          ) : isEndingSoon && (
      ...
      <div style={{ padding: '24px', display: 'flex', flexDirection: 'column', flexGrow: 1, backgroundColor: '#0a0a0a' }}>
        ...
        <h3 style={{ fontSize: '18px', fontWeight: 800, color: isEnded ? '#94a3b8' : '#f1f5f9', margin: '0 0 20px 0', lineHeight: 1.4, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
        ...
        <div style={{ marginTop: 'auto', paddingTop: '20px', borderTop: '1px solid rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'baseline', gap: '4px' }}>
              ...
              <span style={{ fontSize: '24px', fontWeight: 900, color: isEnded ? '#94a3b8' : '#fff' }}>
                {auction.currentBid}
              </span>
            </div>
          </div>
          ...
          {isEnded ? (
             <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end' }}>
               <span style={{ color: '#64748b', fontSize: '10px', textTransform: 'uppercase', fontWeight: 800, letterSpacing: '0.05em', marginBottom: '4px' }}>Winner</span>
               <span style={{ color: '#f59e0b', fontSize: '14px', fontWeight: 800 }}>{winnerName}</span>
             </div>
          ) : (
            <div
              style={{ display: 'flex', alignItems: 'center', gap: '8px', backgroundColor: '#f59e0b', color: '#000', fontSize: '12px', fontWeight: 800, textTransform: 'uppercase', padding: '12px 20px', borderRadius: '12px', boxShadow: '0 4px 12px rgba(245, 158, 11, 0.2)' }}
            >
